/**
 * PROVISIONAL: Run status is via WebSocket/CLI for this OpenClaw instance.
 * This route uses guessed HTTP paths; no standard REST run lifecycle.
 */
import { NextRequest, NextResponse } from "next/server";
import { prisma } from "@/lib/db";
import { getSessionUserId } from "@/lib/auth-server";
import { OpenClawError, getRunStatus } from "@/lib/openclaw/client";
...
export async function GET(
  ...
  try {
    ...
    const result = run.openclawRunId
      ? await getRunStatus(run.openclawRunId)
      : { runId: run.id, status: run.status, message: undefined, raw: undefined };

    const updateData: { lastCheckedAt: Date; status?: string } = {
      lastCheckedAt: new Date(),
    };
    if (result.status != null && result.status.trim() !== "") {
